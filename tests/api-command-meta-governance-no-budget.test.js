...
test("meta-governance draft can submit without budget items", async () => {
  clearIdempotencyForTests();
  clearInlineReadModelsForTests();
  clearProposalDraftsForTests();
  clearProposalsForTests();

  const cookie = await makeSessionCookie(baseEnv, "5MetaGovernor");
  ...
  const saveRes = await commandPost(
    makeContext({
      ...
      body: JSON.stringify({
        ...
        payload: {
          form: {
            title: "Create chamber: Test",
            chamberId: "general",
            summary: "",
            what: "Create the Product chamber in the simulation.",
            why: "We need a specialization chamber for product decisions.",
            how: "Define chamber parameters and seed initial members.",
            metaGovernance: {
              action: "chamber.create",
              chamberId: "test-chamber",
              title: "Test chamber",
              genesisMembers: ["5FGenesisA", "5FGenesisB"],
            },
            timeline: [],
            outputs: [],
            budgetItems: [],
